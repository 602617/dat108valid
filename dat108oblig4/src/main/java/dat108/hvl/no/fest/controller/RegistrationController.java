...
public class RegistrationController {
    ...
    public String submitRegistration(@ModelAttribute("participant") Participant participant,
    	...
    	//if fornavn ikke mellom 2-20 bokstaver og stor bokstav
    	//flashattribute redirect message 
    	if(participant.getFirstname().length() < 2 || Character.isLowerCase()) {
    		ra.addFlashAttribute("message", "must be more than 2 characters");
    		return "redirect:/registration";
    	}
    	
    	if(participant.getLastname().length() < 2) {
    		ra.addFlashAttribute("message2", "must be more than 2 characters");
    		return "redirect:/registration";
    	}
    	
    	if(participant.getPassword().length() < 6) {
    		ra.addFlashAttribute("message3", "must be more than 6 characters");
    		return "redirect:/registration";
    	}
